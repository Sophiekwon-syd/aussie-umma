...
- `OUTPUT_DIR` — the date folder, e.g. `outputs/<BRAND>/<DATE>/run-N/`
- `HTML_FILES` — list of validated HTML file paths to screenshot (will be at `outputs/<BRAND>/<DATE>/run-N/<slug>.html`)
...
- Saves PNGs to `outputs/<BRAND>/<DATE>/run-N/images/<carousel-name>-01.png`, `-02.png`, etc.
...
3. Verify the expected PNG files were created in `outputs/<BRAND>/<DATE>/run-N/images/`
  ...
  "produced": [
    {
      ...
      "html": "outputs/<BRAND>/<DATE>/run-N/carousel-01.html",
      "pngs": [
        "outputs/<BRAND>/<DATE>/run-N/images/carousel-01-01.png",
        "outputs/<BRAND>/<DATE>/run-N/images/carousel-01-02.png"
      ],
      "card_count": 10
    }
  ],
...
- PNG output directory (`outputs/<BRAND>/<DATE>/run-N/images/`) is created automatically by the script
- All carousels for a given run share the same `images/` folder, distinguished by filename prefix
